    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if rotated:
                ...
                ##############################################################
                alpha = 0.8  ########### 透明度 数值越小 透明度越高 ##############
                #############################################################
                cv2.rectangle(overlay, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)

            if label:   ################################ 字体大小调整 fontScale=self.sf / 3 分母 ##########################
                w, h = cv2.getTextSize(label, 0, fontScale=self.sf / 1, thickness=self.tf)[0]  # text width, height
                outside = p1[1] - h >= 3
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    if 0 < len(names) < 30:
        ...
        # ax[0].set_xticklabels(list(names.values()), rotation=90, fontsize=14) # x轴字体大小
    else:
